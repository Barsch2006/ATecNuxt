...
class Session implements WithId<ISession> {
    ...
    static async createSession(
        ...
        try {
            ...
            const collection = atec.database.collection("sessions");
    ...
    static async findSession(token: string): Promise<MethodResult<Session>> {
        try {
            const atec =
                ATecManager.instance ?? (await ATecManager.init(useRuntimeConfig()));
            const collection = atec.database.collection<ISession>("sessions");
    ...
    static async tick() {
        ...
        const sessionCollection = atec.database.collection<ISession>("sessions");
    ...
    async activate(): Promise<MethodResult<boolean>> {
        ...
        try {
            const atec =
                ATecManager.instance ?? (await ATecManager.init(useRuntimeConfig()));
            const dbResult = await atec.database
                .collection("sessions")
    ...
    async destroy(): Promise<MethodResult<boolean>> {
        ...
        try {
            const atec =
                ATecManager.instance ?? (await ATecManager.init(useRuntimeConfig()));
            const dbResult = await atec.database.collection("sessions").deleteOne({
                _id: this._id,
            });
